...
describe("beat-game behavior programs", () => {
  ...
  it("carves and follows each level of a descending staircase", async () => {
    ...
    expect(driver.paths.map(({ position, radius }) => ({
      position,
      radius,
    }))).toEqual([{ position: from, radius: 0 }]);
  ...
  it("uses a broad detour when the staircase is deeper than its span", async () => {
    ...
    expect(driver.paths.map(({ position }) => position)).toEqual([from]);
    expect(driver.actions.filter((action) =>
      action.type === "set-movement" && action.forward === true
    ))
      .toHaveLength(6);
    expect(driver.actions.filter((action) => action.type === "look").every(
      (action) =>
        Math.abs(action.yaw / 90 - Math.round(action.yaw / 90)) < 1e-9,
    )).toBe(true);
  });
  ...
  it("retreats uphill before bridging an open staircase tread", async () => {
    const driver = new FakeBeatGameDriver();
    const from = {
      x: 0,
      y: 4,
    ...
    let treadPlaced = false;
    let supportPlaced = false;
    let retreatedBeforePlacement = false;
    driver.currentObservation = observation({
      counts: { "minecraft:cobblestone": 2 },
    });
    driver.actionObserver = (action) => {
      if (
        ...
        retreatedBeforePlacement = driver.paths.at(-1)?.position === from;
        treadPlaced = true;
      }
      if (
        action.type === "place-block"
    ...
    expect(retreatedBeforePlacement).toBe(true);
  });
  ...
  it("prefers observed portal geometry over a conflicting axis property", async () => {
    ...
    driver.observationResolver = () =>
      Effect.sync(() => {
        const dimension = driver.paths.length > 0
          ? "minecraft:overworld"
          : "minecraft:the_nether";
        return observation({
          dimension,
          position: driver.paths.length > 0
            ? {
              x: 11,
              y: 64,
              z: 20.5,
  ...
  it("places a raised portal approach before entering its path", async () => {
    ...
    driver.observationResolver = () =>
      Effect.sync(() => {
        ...
        const dimension = driver.paths.length >= 2
          ? "minecraft:the_nether"
          : "minecraft:overworld";
        return observation({
          dimension,
  ...
  it("builds a reachable End portal approach when the bot is below the rim", async () => {
    ...
    driver.blockQueryResolver = ({ center, selector }) => {
      ...
      const solid = position.y <= 29 || placed.has(key);
    ...
    expect(driver.actions.some(({ type }) => type === "place-block")).toBe(
      true,
    );
